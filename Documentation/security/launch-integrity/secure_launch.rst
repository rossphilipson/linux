...
documentation for the ``SKINIT`` instruction. See the :ref:`resources` section
for references.
...
feature. The primary guiding principle was in minimize the modifications to the
...
:ref:`Secure Launch Resource Table <secure-launch-resource-table>` (``SLRT``)
to determine both what to measure and into which PCR each measurement should be
extended.
